...
void CPlayScene::_ParseSection_OBJECTS(string line)
{
	...
	case OBJECT_TYPE_MARIO:
		...
		break;
	case OBJECT_TYPE_GOOMBA: obj = new CGoomba(x,y, model); break;
	...
	case OBJECT_TYPE_PORTAL:
	{
		...
		float startX = (float)atof(tokens[5].c_str());
		float startY = (float)atof(tokens[6].c_str());
		int type = atoi(tokens[7].c_str());
		int scene_id = atoi(tokens[8].c_str());
		obj = new CPortal(x, y, r, b,startX, startY, type, scene_id);
	}
	break;
		default:
